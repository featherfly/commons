...
public final class Randoms {
     ...
     * @return 一个随机大写字母
     */
    public static char getLetterUpperCase() {
     ...
     * @return 一个随机小写字母
     */
    public static char getLetterLowerCase() {
     ...
     * @return 一个给定长度的随机字符串
     */
    public static String getString(int length, CharType... charTypes) {
        StringBuilder result = new StringBuilder();
        ...
        if (signList.size() > 0) {
            for (int i = 0; i < length; i++) {
                ...
                if (index == 0) {
                    result.append(getLetterLowerCase());
                } else if (index == 1) {
                    result.append(getLetterUpperCase());
                } else {
     ...
     * @return 随机选择的对象
     */
    public static <T> T get(List<T> source) {
        checkSource(source);
        int index = getInt(source.size());
     ...
     * @return 随机选择的对象
     */
    public static <T> String getString(List<T> source, int length) {
        checkSource(source);
     ...
     * @return 随机数组
     */
    public static double[] getFromTotal(double total, int num) {
        ...
        while (leftMoneyPackage.remainSize > 0) {
            results[i] = getMoney(leftMoneyPackage);
            i++;
        }
        return results;
    }

    private static double getMoney(LeftMoneyPackage _leftMoneyPackage) {
