...
public interface ChainMap<K, V> extends Map<K, V> {
     ...
     * @return 当前MAP
     */
    default ChainMap<K, V> set(K key, V value) {
        return putChain(key, value);
    }

    /**
     * 带链式调用的putAll方法. {@link java.util.Map#putAll(Map)}.
     *
     * @param map 要在此映射中存储的映射关系
     * @return 当前MAP
     */
    default ChainMap<K, V> set(Map<? extends K, ? extends V> map) {
        return putAllChain(map);
    }
