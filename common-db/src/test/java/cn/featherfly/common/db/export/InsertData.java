...
public class InsertData {
    ...
    public static void insertDep(DataSource dataSource, int num) throws SQLException {
        ...
        for (int i = 0; i < num; i++) {
            ...
            String sql = String.format(
                    "insert into sys_department (DEPARTMENT_ID , DEPARTMENT_NAME, create_time) values (null, '%s' , '%s')",
    ...
    public static void insertOtop(DataSource dataSource, int num) throws SQLException {
        ...
        long start = System.currentTimeMillis();
        for (int i = 0; i < num; i++) {
            String sql = String.format("insert into oto_p2 (id , name) values (null, '%s')", "name_insert_" + i);
            sta.addBatch(sql);
        }
        sta.executeBatch();
        long end = System.currentTimeMillis();
    ...
    public static void insertUser(DataSource dataSource, int num) throws SQLException {
        ...
        long start = System.currentTimeMillis();
        for (int i = 0; i < num; i++) {
            String sql = String.format("insert into user values (null, '%s', %s)", "name_insert_" + i, i);
            sta.addBatch(sql);
        }
        sta.executeBatch();
        long end = System.currentTimeMillis();
    ...
    public static void insertPerson(DataSource dataSource, int num, boolean withDate) throws Exception {
        ...
        if (withDate) {
            SimpleDateFormat format = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
            for (int i = 0; i < num; i++) {
                String sql = String.format("insert into person values (null, '%s', '%s')", "p_name_insert_" + i,
                        format.format(new Date()));
                sta.addBatch(sql);
            }
        } else {
            for (int i = 0; i < num; i++) {
                String sql = String.format("insert into person values (null, '%s', null)", "p_name_insert_" + i);
                sta.addBatch(sql);
            }
        }
        sta.executeBatch();
