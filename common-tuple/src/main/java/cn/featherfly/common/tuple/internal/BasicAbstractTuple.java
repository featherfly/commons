...
import cn.featherfly.common.tuple.BasicTuple;
...
public abstract class BasicAbstractTuple<T extends BasicTuple<R>, R> implements BasicTuple<R> {
    ...
    BasicAbstractTuple(Class<? extends T> baseClass, Object... values) {
        ...
        if (values.length != degree()) {
            throw new IllegalArgumentException("A Tuple of degree " + degree() + " must contain exactly " + degree()
    ...
    protected int assertIndexBounds(int index) {
        if (index < 0 || index >= degree()) {
            throw new IndexOutOfBoundsException(
                "index " + index + " is illegal. The degree of this Tuple is " + degree() + ".");
        }
        return index;
    }
    ...
    public String toString() {
        return getClass().getSimpleName() + " "
            + Stream.of(values).map(Objects::toString).collect(joining(", ", "{", "}"));
    }
